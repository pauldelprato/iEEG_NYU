function plot_avgHGP_wide(avg_data, params,channels, visible)
...
% Plot 1 channel per figure
chan_length = length(channels);
plot_length = length(channels);
% if chan_length > plot_length*5
...
for iplot = 1:plot_length
    ...
    if iplot == plot_length && rem(chan_length,1)~=0
        panel_num = rem(chan_length,1);
    else
        panel_num = 1;
    end
